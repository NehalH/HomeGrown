...
public class MainActivity extends AppCompatActivity {
    ...
    private long backPressedTime; // Variable to track back button presses
    ...
    public void onBackPressed() {
        if (searchEditText.getText().toString().isEmpty()) {
            // If the search field is empty, proceed with default back button behavior
            if (backPressedTime + 2000 > System.currentTimeMillis()) {
                // If the time interval between back button presses is less than 2 seconds, close the app
                super.onBackPressed();
                finishAffinity();
            } else {
                // Otherwise, show a toast message indicating that another press is required to exit
                Toast.makeText(this, "Press back again to exit", Toast.LENGTH_SHORT).show();
            }
            backPressedTime = System.currentTimeMillis();
        } else {
            // Clear the search field and retrieve all products
