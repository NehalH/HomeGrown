...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void uploadImage(String documentId) {
        if (imageUri != null) {
            // Get a reference to the user's image directory in Cloud Storage
            StorageReference userDirectoryRef = storageReference.child("users");

            // Create the "users" directory if it doesn't exist
            userDirectoryRef.child(documentId).putFile(Uri.EMPTY)
                    .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                        @Override
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            // Directory creation successful or already exists
                            // Continue with uploading the image file
                            StorageReference imageRef = userDirectoryRef.child(documentId).child("profile_image.jpg");
                        ...
                        public void onFailure(@NonNull Exception e) {
                            ...
                            Toast.makeText(RegisterActivity.this, "Failed to create user directory: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    });
        } else {
            // No image selected, proceed with registration without uploading image
